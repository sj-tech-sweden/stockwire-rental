...
The plan below remains useful for sequencing, but several Phase 4/5 capabilities are now already present in baseline form:

- Eventory integration baseline with multi-instance configuration.
- Storage abstraction with local disk and S3-compatible backends.
- Attachments module used by company/jobs/products/devices/maintenance.
- Offline queue UI and flush/retry/blocked handling baseline.
- SSO configuration baseline (OIDC/SAML + role mapping flows).

Open work should focus on depth and hardening: UX polish, scalability, observability, and test coverage expansion.
