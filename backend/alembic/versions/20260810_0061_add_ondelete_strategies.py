...
def _drop_existing_fk(
    table_name: str, constrained_columns: list[str], referred_table: str, *, required: bool = True
) -> bool:
    ...
    if required:
        raise RuntimeError(
            f"Could not find foreign key on {table_name}({', '.join(constrained_columns)}) "
...
def upgrade() -> None:
    ...
    _drop_existing_fk("device_maintenance", ["device_id"], "devices")
    op.create_foreign_key("device_maintenance_device_id_fkey", "device_maintenance", "devices", ["device_id"], ["id"], ondelete="CASCADE")

    _drop_existing_fk("maintenance_comments", ["created_by_user_id"], "users")
    ...
    _drop_existing_fk("defect_reports", ["created_by_user_id"], "users")
    ...
    _drop_existing_fk("defect_comments", ["created_by_user_id"], "users")
    ...
    _drop_existing_fk("job_crew_assignments", ["crew_member_id"], "crew_members")
...
def downgrade() -> None:
    ...
    op.create_foreign_key("fk_device_maintenance_device_id_devices", "device_maintenance", "devices", ["device_id"], ["id"])

    op.drop_constraint("devices_parent_component_device_id_fkey", "devices", type_="foreignkey")
    op.create_foreign_key("fk_devices_parent_component_device_id", "devices", "devices", ["parent_component_device_id"], ["id"], ondelete="SET NULL")

    op.drop_constraint("devices_case_device_id_fkey", "devices", type_="foreignkey")
    op.create_foreign_key("fk_devices_case_device_id", "devices", "devices", ["case_device_id"], ["id"], ondelete="SET NULL")

    op.drop_constraint("devices_location_zone_id_fkey", "devices", type_="foreignkey")
    op.create_foreign_key("fk_devices_location_zone_id_zones", "devices", "zones", ["location_zone_id"], ["id"])
    ...
    op.create_foreign_key("fk_products_category_id_inventory_categories", "products", "inventory_categories", ["category_id"], ["id"])
